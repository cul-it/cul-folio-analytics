...
DROP TABLE IF EXISTS local_shared.sm_holdings_notes;

CREATE TABLE local_shared.sm_holdings_notes AS
SELECT
    ...
    jsonb_extract_path_text(h.jsonb, 'metadata', 'createdByUserId')::uuid AS note_created_by_user_id,
    jsonb_extract_path_text(h.jsonb, 'metadata', 'updatedDate')::timestamptz AS note_date_updated,
    jsonb_extract_path_text(h.jsonb, 'metadata', 'updatedByUserId')::uuid AS note_updated_by_user_id
