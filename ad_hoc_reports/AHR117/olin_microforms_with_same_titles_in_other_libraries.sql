-AHR117
--olin_microforms_with_same_titles_in_other_libraries
WITH parameters AS 

(
SELECT
	'Olin Library'::VARCHAR AS owning_library
	-- enter a library name, such as "Olin Library", "Library Annex", "Mann Library" etc. May be left blank for all libraries.
),
...
SELECT
	iid.instance_id,
	STRING_AGG (DISTINCT SUBSTRING (iid.identifier,
	'^\d{8,13}[X]{0,1}'),
	' | ') AS normalized_isbn_number
FROM
	folio_reporting.instance_identifiers AS iid
WHERE
	iid.identifier_type_name = 'ISBN'
GROUP BY
	iid.instance_id 
),
...
SELECT
	...
	STRING_AGG (DISTINCT SUBSTRING (iid.identifier,
	'\d{1,}'),
	' | ') AS normalized_oclc_number
FROM
	folio_reporting.instance_identifiers AS iid
WHERE
	iid.identifier LIKE '%(OCoLC)%'
GROUP BY
	iid.instance_id
),
...
SELECT
	DISTINCT
		...
		ELSE 'Other microformat'
	END AS microform_type,
	...
	he.call_number,
	he.call_number_suffix)) AS whole_call_number,
	STRING_AGG (DISTINCT he.type_name,
	' | ') AS holdings_type_name,
	STRING_AGG (DISTINCT ic.contributor_name,
	' | ') AS contributors,
	STRING_AGG (DISTINCT ip.publisher,
	' | ') AS publisher,
	ii.publication_period__start::VARCHAR,
	ii.publication_period__end::VARCHAR,
	STRING_AGG (DISTINCT iseries.series,
	' | ') AS series,
	STRING_AGG (DISTINCT hs.statement,
	' | ') AS holdings_statements,
	string_agg (DISTINCT hs.public_note,
	' | ') AS holdings_public_notes,
	STRING_AGG (DISTINCT hss.statement,
	' | ') AS holdings_supplements,
	STRING_AGG (DISTINCT hsi.statement,
	' | ') AS "indexes",
	oclcs.normalized_oclc_number,
	isbns.normalized_isbn_number,
	he.receipt_status,
	CASE
		WHEN he.created_date::date >= '2021-06-20' THEN he.created_date::date
		ELSE mm.create_date::date
	END AS holdings_create_date,
	instlang.language AS primary_language
FROM
	inventory_instances AS ii
LEFT JOIN srs_marctab AS sm 
        ON
	ii.hrid = sm.instance_hrid
LEFT JOIN folio_reporting.instance_series AS iseries
        ON
	ii.id = iseries.instance_id
LEFT JOIN oclcs 
        ON
	ii.id = oclcs.instance_id
LEFT JOIN isbns 
        ON
	ii.id = isbns.instance_id
...
WHERE
	...
	1) = 'h'
		...
		AND (instlang.language_ordinality = 1
			OR instlang.language_ordinality IS NULL)
	GROUP BY
		ii.hrid,
		he.holdings_id,
		he.holdings_hrid,
...
SELECT
	...
	candidates.holdings_public_notes,
...
SELECT
	DISTINCT
		...
		he.call_number_prefix,
		he.call_number,
		he.call_number_suffix)) ILIKE '%film%' THEN 'Microfilm'
		WHEN trim (concat_ws (' ',
		he.call_number_prefix,
		he.call_number,
		he.call_number_suffix)) ILIKE '%fiche%' THEN 'Microfiche'
		ELSE 'Other microformat'
	END AS microform_type,
	ii.hrid AS instance_hrid,
	--
	...
	he.receipt_status,
	CASE
		WHEN he.created_date::DATE >'2021-06-19' THEN he.created_date::DATE
		ELSE mm.create_date::DATE
	END AS holdings_create_date,
	ie.created_date::DATE AS item_create_date,
	instlang.language AS primary_language,
	CASE
		WHEN ie.created_date::DATE IS NULL THEN he.created_date::DATE
		ELSE ie.created_date::DATE
	END AS record_create_date
FROM
	inventory_instances AS ii
LEFT JOIN folio_reporting.instance_series AS iseries 
                ON
...
WHERE
	...
	he.call_number_prefix,
	he.call_number,
	he.call_number_suffix)) SIMILAR TO '%(fiche|film|Fiche|Film|Micro|micro)%')
...
SELECT
	...
	STRING_AGG (DISTINCT final.microform_type,
	' | ') AS microform_type,
	final.instance_hrid,
	final.holdings_hrid,
	final.library_name,
	...
	final.holdings_public_notes,
	final.holdings_supplements,
	final."indexes",
	final.normalized_oclc_number,
	final.normalized_isbn_number,
	final.primary_language,
	STRING_AGG (DISTINCT final.source,
	' | ') AS sources,
	STRING_AGG (DISTINCT final.total_items::VARCHAR,
...
LEFT JOIN srs_marctab AS sm 
        ON
	final2.instance_hrid = sm.instance_hrid
WHERE
	sm.field = '007'
GROUP BY
	final2.instance_hrid
...
SELECT
	final2.instance_hrid,
	string_agg (DISTINCT substring (iniden.identifier,
	'\d{1,}'),
	' | ') AS oclc_number
FROM
	final2
LEFT JOIN folio_reporting.instance_identifiers AS iniden
        ON
	final2.instance_hrid = iniden.instance_hrid
WHERE
	iniden.identifier_type_name = 'OCLC'
GROUP BY
	final2.instance_hrid,
	iniden.identifier
),
...
SELECT
	...
	STRING_AGG (DISTINCT final2.microform_type,
	' | ') AS microform_type,
	final2.instance_hrid,
	...
	final2.holdings_public_notes,
	final2.holdings_supplements,
	STRING_AGG (DISTINCT inotes.note,
	' | ') AS instance_note,
	STRING_AGG (DISTINCT hn.note,
	' | ') AS holdings_note,
	SUBSTRING (STRING_AGG (DISTINCT hn.note,
	' | '),
	'31924\d{9}') AS parent_barcode,
	CASE
		WHEN STRING_AGG (DISTINCT hn.note_type_name,
		' | ') LIKE '%Bound with%' THEN 'Bound with'
		ELSE '-'
	END AS bound_with,
	final2."indexes",
	...
	final2.total_items,
	"007f"."007",
	string_agg (DISTINCT final2.normalized_oclc_number,
	' | ') AS oclc_number,
	--"035f".oclc_number,
        "074f"."074",
	"086f"."086",
	...
	"533f"."533e",
	CASE
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'a' THEN 'Silver halide'
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'b' THEN 'Diazo'
		WHEN SUBSTRING ("007f"."007",
		...
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'n' THEN 'Not applicable'
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'u' THEN 'Unknown'
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'm' THEN 'Mixed emulsion'
		WHEN SUBSTRING ("007f"."007",
		11,
		1) = 'z' THEN 'Other'
		WHEN SUBSTRING ("007f"."007",
		...
		1) = '|' THEN 'No attempt to code'
		ELSE ' - '
	END AS emulsion_type,
	STRING_AGG (DISTINCT ll.location_name,
	' | ') AS all_holdings_locations
FROM
	final2
LEFT JOIN "007f"
        ON
	"007f".instance_hrid = final2.instance_hrid
LEFT JOIN "035f" 
        ON
	"035f".instance_hrid = final2.instance_hrid
LEFT JOIN "074f" 
        ON
	"074f".instance_hrid = final2.instance_hrid
LEFT JOIN "086f"
        ON
	"086f".instance_hrid = final2.instance_hrid
LEFT JOIN "300f" 
        ON
	"300f".instance_hrid = final2.instance_hrid
LEFT JOIN "533f" 
        ON
	"533f".instance_hrid = final2.instance_hrid
LEFT JOIN folio_reporting.instance_notes AS inotes 
        ON
	final2.instance_hrid = inotes.instance_hrid
LEFT JOIN folio_reporting.holdings_notes AS hn 
        ON
	final2.holdings_hrid = hn.holdings_hrid
LEFT JOIN inventory_instances AS ii 
        ON
	final2.instance_hrid = ii.hrid
LEFT JOIN folio_reporting.holdings_ext AS he 
        ON
	ii.id = he.instance_id
LEFT JOIN folio_reporting.locations_libraries AS ll 
	...
	he.permanent_location_id = ll.location_id
WHERE
	final2.microform_type ILIKE '%fiche%'
GROUP BY
	final2.instance_hrid,
	final2.holdings_hrid,
	final2.whole_call_number,
