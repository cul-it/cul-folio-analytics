...
--Updated on 3/20/24 to filter and link by location: Linda Miller (lm15)
...
holdings AS
    (SELECT 
       ...
       il.code AS holdings_permanent_location_code, 
      up.unpurchased,
      mc.micro_by_007
...
(SELECT DISTINCT
    ...
    fmg.folio_format_type_acrl_nces_groups
       ...
       LEFT JOIN local_core.lm_adc_location_translation_table AS adc2 ON hh.holdings_permanent_location_code = adc2.adc_invloc_location_code
       )
