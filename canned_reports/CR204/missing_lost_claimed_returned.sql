...
WITH parameters AS (
    SELECT
        ...
        ''::varchar AS item_status_filter -- enter one of: Missing, Aged to lost, Lost and paid, Declared lost, Claimed returned, Unavailable; or leave blank to get all missing/lost/claimed returned/unavailable statuses
),
recs AS (
    SELECT
        ll.library_name,
        ...
        instext.title,
        TRIM(CONCAT_WS(' ', he.call_number_prefix, he.call_number, he.call_number_suffix, ii.enumeration, ii.chronology, CASE WHEN ii.copy_number > '1' THEN
                    concat('c.', ii.copy_number)
                ELSE
                    ''
    ...
    CASE WHEN ii.last_check_in__date_time IS NULL THEN
        NULL
    ELSE
        TO_CHAR(ii.last_check_in__date_time::timestamp, 'mm/dd/yyyy hh:mi am')
    END AS last_folio_check_in,
...
FROM
    folio_reporting.instance_ext AS instext
    LEFT JOIN folio_reporting.holdings_ext AS he ON instext.instance_id = he.instance_id
        ...
        LEFT JOIN vger.circ_trans_archive AS cta ON ii.hrid::varchar = cta.item_id::varchar
        LEFT JOIN inventory_service_points AS isp ON ii.last_check_in__service_point_id = isp.id
        LEFT JOIN folio_reporting.item_notes AS itemnotes ON itemext.item_id = itemnotes.item_id
        LEFT JOIN srs_marctab AS srs ON instext.instance_hrid = srs.instance_hrid
    WHERE (ll.library_name = (
            SELECT
                owning_library_name_filter
            FROM
                parameters)
            OR (
                SELECT
...
loan1 AS (
    ...
    FROM
        recs
        LEFT JOIN folio_reporting.loans_items AS li ON recs.item_id = li.item_id
    GROUP BY
        recs.item_hrid,
        recs.item_id
),
loan2 AS (
    ...
    FROM
        loan1
        LEFT JOIN folio_reporting.loans_items AS li ON loan1.most_recent_loan = li.loan_date
            AND loan1.item_id = li.item_id
        LEFT JOIN user_users AS uu ON li.user_id = uu.id
)
SELECT
    ...
    CASE WHEN loan2.patron_status = 'True' THEN
        'Active'
    WHEN loan2.patron_status = 'False' THEN
        'Expired'
    ELSE
        ' - '
...
FROM
    recs
    LEFT JOIN vger.circ_trans_archive AS cta ON recs.last_voyager_check_in = TO_CHAR(cta.discharge_date::timestamp, 'mm/dd/yyyy hh:mi am')
        AND recs.item_hrid = cta.item_id::varchar
    LEFT JOIN vger.location ON cta.discharge_location = location.location_id
    LEFT JOIN loan2 ON recs.item_hrid = loan2.item_hrid
WHERE
    recs.status_name = (
        SELECT
            item_status_filter
