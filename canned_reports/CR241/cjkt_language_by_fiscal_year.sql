...
-- cjkt_language_by_fiscal_year
-- query written and tested by Joanne Leary and Sharon Markus
-- This query finds all item or holdings records in Chinese, Japanese, Korean and Tibetan languages 
-- that were added to the collection in the fiscal year specified. All libraries and locations are included. 
-- Counts are sorted by language and month added.The instance format is extracted FROM the leader, 
...
-- WHERE clause on line 148 to  '>=' instead of '='
-- The results of this query are combined with the results of CR242 cjkt_subject_by_fiscal_year to provide
...
(SELECT DISTINCT
    ...
    CASE 
	    ...
	    WHEN 
		    (CONCAT_WS (' ',he.call_number_prefix,he.call_number,he.call_number_suffix) 
		    SIMILAR TO '%(Film|Fiche|Micro|film|fiche|micro|FILM|FICHE|MICRO)%'
...
WHERE 
	...
	AND instlang.language IN ('chi','jpn','kor','tib')
	...
	AND ii.hrid NOT IN 
		(SELECT DISTINCT sm.instance_hrid FROM srs_marctab AS sm WHERE sm.field='856' 
	    AND sm.content ILIKE '%Not yet in permanent collection%')
	)
