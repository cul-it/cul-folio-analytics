...
--publication update
publication_extract AS ( -- gets publication date, place, and publisher from the inventory_instances table
       SELECT 
               instance.id AS instance_id,
               instance.hrid AS instance_hrid,
               publication.data #>> '{dateOfPublication}' AS publication_date,
               publication.data #>> '{place}' AS publication_place,
...
WHERE (contributors.data #>> '{primary}')::boolean = 'True'
),
...
--publication update with regular expression to format as date with 4 digits, stripping out everything else
