...
(SELECT 
  ...
  FROM folio_reporting.instance_statistical_codes isc
  LEFT JOIN folio_reporting.instance_ext ii ON isc.instance_id =ii.instance_id
  LEFT JOIN inventory_instances i ON i.id = isc.instance_id 
     CROSS JOIN jsonb_array_elements((i.data #> '{publication}')::jsonb) 
...
(SELECT 
  ...
  FROM folio_reporting.instance_statistical_codes isc
  LEFT JOIN folio_reporting.instance_ext ii ON isc.instance_id =ii.instance_id
  LEFT JOIN srs_marctab sm ON sm.instance_id::uuid= isc.instance_id::uuid
...
LEFT JOIN srs_marctab sm ON aa.inst_hrid=sm.instance_hrid 
LEFT JOIN folio_reporting.holdings_ext he ON aa.inst_id = he.instance_id 
WHERE (sm.field = '008')
