...
DROP TABLE IF EXISTS LOCAL.h_s_1; 
CREATE TABLE LOCAL.h_s_1 AS
SELECT
...
;
CREATE INDEX ON local.h_s_1 (instance_hrid);
CREATE INDEX ON local.h_s_1 (instance_id);
CREATE INDEX ON local.h_s_1 (field);
CREATE INDEX ON local.h_s_1 (sf);
CREATE INDEX ON local.h_s_1 (content);
CREATE INDEX ON local.h_s_1 (permanent_location_name);
CREATE INDEX ON local.h_s_1 (call_number);
CREATE INDEX ON local.h_s_1 (discovery_suppress);

--2------------selects/deselects records with 945 (monoseries standing orders) and filters it from h_s_1 table-------------------------------------
DROP TABLE IF EXISTS local.h_s_2;
CREATE TABLE LOCAL.h_s_2 AS
...
(SELECT
    ...
    hsr.call_number 
    FROM LOCAL.h_s_1 hsr
    LEFT JOIN ninefortyfive n ON hsr.instance_id = n.instance_id
    WHERE n.instance_id IS NULL
;
CREATE INDEX ON local.h_s_2(instance_hrid);
CREATE INDEX ON local.h_s_2(instance_id);
CREATE INDEX ON local.h_s_2(permanent_location_name);
CREATE INDEX ON local.h_s_2(call_number);

--3------------------------selects/deselects records with 245 $h[electronic resource] and filters from h_s_2 tabe------------------------
DROP TABLE IF EXISTS LOCAL.h_s_3;   
CREATE TABLE LOCAL.h_s_3 AS 
...
;
CREATE INDEX ON local.h_s_3 (instance_id);
CREATE INDEX ON local.h_s_3 (instance_hrid);
CREATE INDEX ON local.h_s_3 (permanent_location_name);
CREATE INDEX ON local.h_s_3 (call_number);

--4---------------------selects/deselects records with 336 $aunmediated content and filters from h_s_3----------------  
DROP TABLE IF EXISTS LOCAL.h_s_4;
CREATE TABLE LOCAL.h_s_4 AS
...
SELECT 
    ...
    WHERE t.instance_id IS NULL
;
CREATE INDEX ON local.h_s_4 (instance_id);
CREATE INDEX ON local.h_s_4 (instance_hrid);
CREATE INDEX ON local.h_s_4 (permanent_location_name);
CREATE INDEX ON local.h_s_4 (call_number);

--5--------------------selects/deselects records with 300 $amap or maps and filters from h_s_4------------------ 
DROP TABLE IF EXISTS LOCAL.h_s_5;
CREATE TABLE LOCAL.h_s_5 AS
...
(SELECT
    ...
    WHERE t.instance_id IS NULL
;
CREATE INDEX ON local.h_s_5 (instance_id);
CREATE INDEX ON local.h_s_5 (instance_hrid);
CREATE INDEX ON local.h_s_5 (permanent_location_name);
CREATE INDEX ON local.h_s_5 (call_number);
CREATE INDEX ON LOCAL.h_s_5 (discovery_suppress);
--6-----------------------filters suppressed holding records with the note "decision - no"-----------
DROP TABLE IF EXISTS LOCAL.h_s_6;
CREATE TABLE LOCAL.h_s_6 AS
...
CREATE INDEX ON local.h_s_6 (instance_id);
CREATE INDEX ON local.h_s_6 (instance_hrid);
CREATE INDEX ON local.h_s_6 (permanent_location_name);
CREATE INDEX ON local.h_s_6 (call_number);
CREATE INDEX ON local.h_s_6 (discovery_suppress);
;

--7-----------------------filters records by certain values in call number from h_s_6-------------------
DROP TABLE IF EXISTS LOCAL.h_s_7;
CREATE TABLE LOCAL.h_s_7 AS
SELECT 
    hhn.instance_hrid,
    hhn.instance_id,
    hhn.call_number,
    hhn.permanent_location_name 
    FROM LOCAL.h_s_6 hhn  
...
;
CREATE INDEX ON local.h_s_7 (instance_hrid);
CREATE INDEX ON local.h_s_7 (instance_id);
CREATE INDEX ON local.h_s_7 (call_number);
CREATE INDEX ON local.h_s_7 (permanent_location_name);

--8--------------------filters records for presents of oclc number------------------    
DROP TABLE IF EXISTS LOCAL.h_s_8;
CREATE TABLE LOCAL.h_s_8 AS
WITH oclc_no AS (
    SELECT
    ...
    CASE 
        ...
        END AS oclc_no
    FROM LOCAL.h_s_7 hsn 
    INNER JOIN oclc_no AS oclcno ON hsn.instance_id = oclcno.instance_id
;
CREATE INDEX ON local.h_s_8 (instance_id);
CREATE INDEX ON local.h_s_8 (instance_hrid);
CREATE INDEX ON local.h_s_8 (call_number);
CREATE INDEX ON local.h_s_8 (permanent_location_name);
CREATE INDEX ON local.h_s_8 (id_type);
CREATE INDEX ON local.h_s_8 (oclc_number2);
CREATE INDEX ON local.h_s_8 (oclc_no);

--9------------------------------selects issn number and value for government document from 008 --------------
DROP TABLE IF EXISTS LOCAL.h_s_final;
CREATE TABLE LOCAL.h_s_final AS
WITH gov_doc AS (
    SELECT
...
;
CREATE INDEX ON local.h_s_final (oclc_no);
CREATE INDEX ON local.h_s_final (instance_hrid);
CREATE INDEX ON local.h_s_final (issn_no);
CREATE INDEX ON local.h_s_final (gov_doc);
