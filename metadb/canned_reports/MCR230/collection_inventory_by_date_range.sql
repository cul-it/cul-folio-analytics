...
-- This query finds all items in a given location or library, and shows all charges and browses within the specified date range. It shows all record component locations, which is helpful for record cleanup. For Folio loan counts > 0, "most recent checkout" dates may be before 7-1-21 because of migrated open loans from Voyager

Query writer: Joanne Leary (jl41)
Query tester: Sharon Markus (slm5)
  

with parameters as 
(select 
'2024-10-01'::date as begin_date_filter, -- required; to get all loans, enter '2000-01-01'
'2024-11-01'::date as end_date_filter, -- required

-- Enter the library name (will get all records) or the location name (will get records just in that location). Required.

'%Vet%'::varchar as library_filter,
'%Path%'::varchar as location_name_filter -- will be used to select holdings or item locations
),

--1. Get holdings or item records in location and show Voyager historical charges and browses 
...
from folio_inventory.instance__t as ii --inventory_instances as ii 
	...
	on ie.item_id::UUID = invitems.id::UUID
...
(select 
	...
	max (cci.occurred_date_time)::DATE as most_recent_browse
	
	from recs 
		left join folio_circulation.check_in__t as cci --circulation_check_ins as cci 
		on recs.item_id::UUID = cci.item_id
		
	where (cci.occurred_date_time::DATE >= (select begin_date_filter from parameters) and cci.occurred_date_time::DATE < (select end_date_filter from parameters))
		and 
		cci.item_status_prior_to_check_in = 'Available'
	group by recs.item_id::UUID
)
...
from recs 	
	left join circs 
	on recs.item_id::UUID = circs.item_id
	
	left join browses 
	on recs.item_id::UUID = browses.item_id
