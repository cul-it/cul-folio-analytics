...
-- Create table based on local_hathitrust.z_metadbuser_cornell_ALL_903s where the subfields of sf_content are parsed out to individual fields.
--
...
select
    ...
    substring(sf_content, ((position('$a' in sf_content))+3),(position('$b' in sf_content))-5) as repo,
    ...
    case
        when position('$h' in sf_content) > 0 then substring(sf_content, ((position('$h' in sf_content))+3),(position('$n' in sf_content)-((position('$h' in sf_content))+3)))
    else null
    end as foreign_id,
    -- parse out persistent ID in the preservation repository ($n)
    case
        when position('$v' in sf_content) > 0 then substring(sf_content, ((position('$n' in sf_content))+3),(position('$v' in sf_content)-((position('$n' in sf_content))+3)))
        else substring(sf_content, ((position('$n' in sf_content))+3))
    end as pres_id,
    --  parse out volume conditionally if it exists ($v)
    CASE
        WHEN position('$v' in sf_content) > 0 THEN  substring(sf_content, (position('$v' in sf_content))+2)
    ELSE null
    end as volume
...
--Use the table local_hathitrust.z_metadbuser_cornell_all_903s_parsed as a foundation, and add HTIDs in a new table local_hathitrust.local_digpres.metadb_903_with_htid
