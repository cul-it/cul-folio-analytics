...
select distinct
	...
	case when he.call_number_type_name = 'Library of Congress classification' then null else substring (he.call_number,'\d{1,}')::int end as vhs_number,	
	ie.barcode,
...
where sm.field = '000'
	...
	and (ipd.physical_description ilike '%cassette%' or ipd.physical_description ilike '%tape%')
	and he.call_number not similar to '%(On order|On Order|In Process|In process|in process|on order)%'
...
group by 
	...
	case when he.call_number_type_name = 'Library of Congress classification' then null else substring (he.call_number,'\d{1,}')::int end,
	ie.barcode,
	ie.status_name,
	ie.status_date::date,
...
order by library_name, permanent_location_name, vhs_number, item__t.effective_shelving_order collate "C", title
;
