...
WITH parameters AS (
    SELECT 
         -- Fill out filter ----
         '%Asia%'::varchar AS owning_library_name_filter, -- Examples: Olin Library, Library Annex, etc.
         'Echols'::varchar as location_name_filter --item_ext.effective_location_name -- Examples: Uris
),
...
WHERE  
    --itemext.barcode like '31924131060778' --use to test a specific barcode
...
(SELECT DISTINCT
        ...
        recs.whole_call_number,
 		recs.barcode,
        recs.title,
        contributors.jsonb #>> '{name}' AS primary_author,
...
FROM
    ...
    LEFT JOIN folio_inventory.instance AS instance
	    CROSS JOIN LATERAL jsonb_array_elements(jsonb_extract_path (instance.jsonb,'contributors'))--((instance.data #> '{contributors}')::jsonb) 
